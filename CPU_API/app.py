...
cpu = CPU()  # Instantiate the CPU object

@app.route('/load', methods=['POST'])
def load_program():
    program = request.json['program']  # Get program from JSON data
    cpu.load_program(program)
    return jsonify({'message': 'Program loaded'})
...
def run_program():
    ...
    result = cpu.registers[0]
    memory = cpu.memory
    return jsonify({'result': result, 'memory': memory})
